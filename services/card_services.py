...
import queries.card_queries
from queries import get_historical_tag_data
...
async def get_user_cards(db: AsyncSession, user_id: int, current_user: dict):
    ...
    try:
        ...
        # After authorization checks, delegate to the query implementation
        return await queries.card_queries.get_user_cards(db, user_id, current_user)
...
async def create_user_card(db: AsyncSession, user_id: int, card: dict, current_user: dict):
    ...
    try:
        ...
        # After authorization checks, delegate to the query implementation
        return await queries.card_queries.create_user_card(db, user_id, card, current_user)
...
async def update_user_card(db: AsyncSession, card_id: int, card: dict, current_user):
    """Update an existing card for a user - supports flexible field updates"""
    if not current_user:
        return await error_response("User Not Authorized")
    ...
    try:
        ...
        # After authorization checks, delegate to the query implementation
        return await queries.card_queries.update_user_card(db, card_id, card)
...
async def delete_card(db: AsyncSession, card_id: int, current_user: dict):
    ...
    try:
        ...
        # After authorization checks, delegate to the query implementation
        return await queries.card_queries.delete_card(db, card_id, current_user)
